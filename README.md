...
Максимальный `page_size` равен `100`. Чередование и сортировка по рейтингу
доступны только для endpoint отзывов филиала. Endpoint одного источника
поддерживает `published_date` и `-published_date`.
